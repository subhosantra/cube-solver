...
from cube.rubik.corner import URF, UFL, ULB, UBR, DFR, DLF, DBL, DRB, corner_values
from cube.rubik.edge import UR, UF, UL, UB, DR, DF, DL, DB, FR, FL, BL, BR, edge_values
from cube.rubik import facecube
...
class CubieCube(object):
    ...
    def toFaceCube(self):
        ...
        for i in corner_values:
            ...
            for n in range(3):
                _butya = facecube.FaceCube.cornerFacelet[i][(n + ori) % 3]
                fcRet.f[_butya] = facecube.FaceCube.cornerColor[j][n]
        for i in edge_values:
            ori = self.eo[i]   # Orientation of this cubie
            for n in range(2):
                _butya = facecube.FaceCube.edgeFacelet[i][(n + ori) % 2]
                fcRet.f[_butya] = facecube.FaceCube.edgeColor[self.ep[i]][n]
        return fcRet
